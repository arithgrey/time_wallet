...
<?=construye_header_modal('editar-registro-modal', "Editar" );?>                                 
    ...
    <form id='form-editar' class='form-editar' action="<?=base_url('index.php/api/emp/ingreso/format/json')?>">      
      ...
      <?=n_row_12()?>
        <textarea class='e-descripcion form-control' id='e-descripcion' name='descripcion'>
        </textarea>
      <?=end_row()?>
      <br>       
      <button  id="button-registrar" class="btn btn-default btn_save ">
...
<?=construye_header_modal('registra-ingreso-modal', "Registrar nuevo" );?>                                 
  ...
  <span class='agregar-categoria' title='Añade a tus cuentas  gastos o ingresos frecuentes'> 
        + agregar categoría
  </span>          
  <br>
  ...
  <form method='POST' class='form-ingresos'  id='form-ingresos' action='<?=base_url('index.php/api/emp/registro/format/json/')?>'>
    ...
    <?=n_row_12()?>      
        ...
        <textarea name="descripcion" id="descripcion_registro" class='descripcion_registro form-control'>
        </textarea>
...
<span data-toggle="modal" data-target="#registra-ingreso-modal"    
        title='Añade nuevos gastos realizados'      class='nuevo_ingreso_btn'  id='nuevo_ingreso_btn'>    
      + Registrar gasto
</span>

<span data-toggle="modal" data-target="#tranferencias-modal" 
     title="Mueve los ingresos de una cuenta a otra "  
...
<?=construye_header_modal('modal_config_cuenta', "Configurar cuenta - <span class='cuenta_btn'></span> " );?>
  <div class='places_update_cuenta'>
  </div>
  <form class="form_info_cuenta">    
    <div class='row'>
      <div class="form-group">
        <label class="col-md-4 control-label" for="nombre_cuenta">Nombre cuenta
        </label>  
        <div class="col-md-6">
          <input id="nombre_cuenta_e" name="nombre_cuenta" placeholder="" class="form-control input-md" type="text">      
        </div>
      </div>
    </div>
    ...
    <div class='row'>
      <div class="form-group">
        <label class="col-md-4 control-label" for="descripcion">Información
        </label>
        <div class="col-md-6">                     
          <textarea class="form-control" id="descripcion_cuenta_e" name="descripcion"></textarea>
        </div>
      </div>
    </div>
