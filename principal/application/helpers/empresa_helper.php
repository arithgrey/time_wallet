...
/**/
function create_info_simple_balance($data){
  ...
  if ($gastos > $ingresos ){
    $extra_class='alerta_cuenta';  
  }
  $saldo =  $ingresos - $gastos; 
  $info =  "<span class='info_saldo  resumen_saldos pull-left  ".$extra_class." '> Saldo: " . $saldo ."</span>";
  
  $info .=  "<span class='info_ingresos resumen_saldos pull-right ".$extra_class." '> Gastos: " . $gastos ."</span>";

  $info .=  "<span class='info_ingresos resumen_saldos   pull-right'> Ingresos: " . $ingresos ."</span>";
  return $info;


}
/**/
function menu_cuentas($data){
  ...
  foreach ($data as $row) {
    ...
    $modal_config_cuenta = "data-toggle='modal' 
                            data-target='#modal_config_cuenta'  
                            id='".$id_cuenta."' 
                            info_descripcion = '".$descripcion."'
                            info_nombre_cuente =  '".$nombre_cuenta ."'
                            ";
    $btn_config =  btn_conf( "configurar_cuenta " , $modal_config_cuenta );
    
    $cuentas .=  "<div class='menu_cuenta_enid col-lg-12 ' 
                    id='".$id_cuenta."'  nombre_cuenta = '".$nombre_cuenta."'  
                    title='Pulsa para mostrar la información de esta cuenta -  
                    ".$descripcion."'  >                            
...
/**/
function get_select_tipo_cuenta($name , $class , $id ){   
   
      $tipo_cuenta = array("Personal" , "Familiar" , "De negocio " , "Bancaria " , "De ahorro" , "Otra");
        
      $select =  "<select name='".$name."' class='".$class ."' id='". $id ."'> ";   
      $x = 0;       
      for ($x=0; $x <count($tipo_cuenta); $x++) { 
        $select .= "<option value='". $tipo_cuenta[$x]."'>". $tipo_cuenta[$x]."</option>";
      }
      $select .=  "</select>";
      return $select;    
}
...
function muestra_seccion_ingresos_egresos($perfiles){
    ...
    $id_perfil =  $perfiles[0]["idperfil"];
    //$secciones_admin = ["ingresos_egresos/view_admin" , "ingresos_egresos/view_productos_prociones" ];  
    $secciones_admin = ["ingresos_egresos/view_admin" ];  
...
function create_tmp_select_categorias($data){
    ...
    $options .= "<select class='categoria_ingreso form-control input-sm' id='categoria_ingreso' name='categoria_ingreso'>";    
    foreach ($data as $row) {
