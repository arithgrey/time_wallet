...
class empresamodel extends CI_Model{
  ...
  function registra_cuenta($param){
    ...
    $query_insert =  "INSERT INTO  cuenta(nombre_cuenta , idempresa , descripcion ) 
    VALUES('".$param["nombre_cuenta"]."' ,  '".$param["id_empresa"]."'   ,  '".$param["tipo_cuenta"]."' )"; 
  ...
  function insert_registro($param){
    ...
    $query_insert =  "INSERT INTO registro( nombre , tipo , id_cuenta , cantidad , valor ,  descripcion  )
                      VALUES(
  ...
  /**/
  function get_categoria_catalogo($param){

    $query_get =  "SELECT * FROM 
                  catalogo_productos_servicios 
                  WHERE id_empresa = '".$param["id_empresa"]."' AND 
                  nombre =  '".$param["categoria"]."' LIMIT 1";
    $result=  $this->db->query($query_get);
    return $result->result_array();
